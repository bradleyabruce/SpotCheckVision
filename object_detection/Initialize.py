...
def initialize_raspberry_pi():
    ...
    if device is None:
        ...
        new_device = Device.Device(None, "Camera", device.getLocalIP(), device.getExternalIP(), device.getMacAddress(),
    ...
    # if device exists in database, update the database
        ...
        updated_device = Device.Device(device.DeviceID, device.DeviceName, device.getLocalIP(), device.getExternalIP(),
                                       device.getMacAddress(), datetime.datetime.now(), device.CompanyID,
                                       device.TakeNewImage, device.DeviceStatusID, device.ParkingLotID)
